...
public partial class House
{
    ...
    public void CreateRooms(int width, int height)
    {
        ...
        for (var i = 0; i < total; i++)
        {
            ...
            if(c < Width - 1)
            {
                tmpRoom.Neighbors[DirectionsEnum.East] = CalculateRoomIndex(c + 1, r);
            }
            
            if(c > 0)
            {
                tmpRoom.Neighbors[DirectionsEnum.West] = CalculateRoomIndex(c - 1, r);
            }

            if(r < Height - 1)
            {
                tmpRoom.Neighbors[DirectionsEnum.South] = CalculateRoomIndex(c, r + 1);
            }

            if(r > 0)
            {
                tmpRoom.Neighbors[DirectionsEnum.North] = CalculateRoomIndex(c, r - 1);
            }
